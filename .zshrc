...
export PATH=$PATH:$HOME/rcfiles/bin
...
# C-M-h でチートシートを表示する
cheat-sheet () { zle -M "`cat ~/rcfiles/.zsh/cheat-sheet`" }
zle -N cheat-sheet
...
## alias設定
#
[ -f ~/rcfiles/.zshrc.alias ] && source ~/rcfiles/.zshrc.alias

case "${OSTYPE}" in
# Mac(Unix)
darwin*)
    # ここに設定
    [ -f ~/rcfiles/.zshrc.osx ] && source ~/rcfiles/.zshrc.osx
    ;;
# Linux
linux*)
    # ここに設定
    [ -f ~/rcfiles/.zshrc.linux ] && source ~/rcfiles/.zshrc.linux
    ;;
esac
